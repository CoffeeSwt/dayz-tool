...
<template>
    <div w-full h-9 flex items-center p-2 rounded-lg cursor-pointer
        :class="{ 'bg-light-background-2  text-light-background-1 dark:bg-dark-background-2 dark:text-dark-text-background-2': isActive }"
        duration-100 @click="routeTo">
        <div size-6 mr-2 :class="props.icon"></div>
        <div tracking-wide>{{ props.label }}</div>
    </div>
</template>
